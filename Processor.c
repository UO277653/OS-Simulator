...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	// V2 Ex 2
	interruptVectorTable[CLOCKINT_BIT]=interruptVectorInitialAddress+4; // EXCEPTION_BIT=6
}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case HALT_INST: 
			...
			// V1-Ex 16
			if(registerPSW_CPU < ((unsigned int)1) << EXECUTION_MODE_BIT){
		...
		default : 
			...
			// registerPC_CPU++; V4 Delivery 1 FIX
